...
public class TransferHolder extends BinderViewHolder<TokenTransferData> implements View.OnClickListener
{
                          ...
                          AssetDefinitionService svs)
    {
        ...
        warningView = findViewById(R.id.tx_warning);
        backgroundView = findViewById(R.id.layout_background);
...
//        if (tx == null) { return; }
        ...
        String transactionValue = getEventAmount(data, null);

        if (data.isZeroUAttackSuspicious() && sharedPreferenceRepository.isRiskCheckOn()) {
            warningView.setVisibility(View.VISIBLE);
            backgroundView.setBackgroundColor(getContext().getColor(R.color.warn_background));
        } else {
        ...
        String title = data.eventName.equals("sent") ? "Sent" : "Received";

        CharSequence typeValue = Utils.createFormattedValue(getContext(), title, token);

        type.setText(typeValue);
        address.setText(data.getDetail(getContext(), null, token, itemView));
